...
let citationBlock = document.querySelector('#quote');
let citationAuthor = document.querySelector('#author');
let citationBtn = document.querySelector('#new-citation');

let dernier = 0;

let nombreAleatoire = generateInteger(citationsTable.length);

function generateInteger(max) {
    return Math.floor(Math.random() * Math.floor(max));
}

function addQuote(nombre) {
    citationBlock.innerHTML = citationsTable[nombre][0];
    citationAuthor.innerHTML = citationsTable[nombre][1];
}

addQuote(nombreAleatoire);

citationBtn.addEventListener('click', () => {
    do {
        nombreAleatoire = generateInteger(citationsTable.length);
